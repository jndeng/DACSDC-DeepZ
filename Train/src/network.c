...
network *make_network(int n)
{
	...
	net->con_cost = calloc(1, sizeof(float));
	net->pos_cost = calloc(1, sizeof(float));
    return net;
}
...
void calc_network_cost(network *netp)
{
    network net = *netp;
    int i;
    float sum = 0, con_sum = 0, pos_sum = 0;
    int count = 0;
    for(i = 0; i < net.n; ++i){
        if(net.layers[i].cost){
            sum += net.layers[i].cost[0];
			con_sum += net.layers[i].con_cost[0];
			pos_sum += net.layers[i].pos_cost[0];
            ++count;
        }
    }
    *net.cost = sum / count;
	*net.con_cost = con_sum / count;
	*net.pos_cost = pos_sum / count;
}
...
float train_network(network *net, data d)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float err = train_network_datum(net);
        sum += err;
		con_sum += net->con_cost[0];
		pos_sum += net->pos_cost[0];
    }
	printf("confidence loss: %.6f,\tposition loss: %.6f\n", con_sum/(n*batch), pos_sum / (n*batch));
    return (float)sum/(n*batch);
}
...
{
    int i;
	if (net->con_cost) free(net->con_cost);
	if (net->pos_cost) free(net->pos_cost);
    for(i = 0; i < net->n; ++i){
