...
#include "focal_layer.h"  // added
#include "mix_layer.h"  // added
...
LAYER_TYPE string_to_layer_type(char * type) {
	...
	if (strcmp(type, "[focal]") == 0) return FOCAL;  // added
	if (strcmp(type, "[mix]") == 0) return MIX;		 // added
...
layer parse_yolo(list *options, size_params params)
{
    ...
    l.max_boxes = option_find_int_quiet(options, "max", 1);
    l.jitter = option_find_float(options, "jitter", .3);
...
layer parse_region(list *options, size_params params) {
    int coords = option_find_int_quiet(options, "coords", 4);
    int classes = option_find_int_quiet(options, "classes", 0);
    int num = option_find_int_quiet(options, "num", 1);
	int max_boxes = option_find_int_quiet(options, "max", 1);
    ...
    l.softmax = option_find_int_quiet(options, "softmax", 0);
    l.background = option_find_int_quiet(options, "background", 0);
    l.jitter = option_find_float_quiet(options, "jitter", .2);
    l.rescore = option_find_int_quiet(options, "rescore",0);

    l.thresh = option_find_float_quiet(options, "thresh", .5);
...
layer parse_focal(list *options, size_params params) {
	int coords = option_find_int_quiet(options, "coords", 4);
	int classes = option_find_int_quiet(options, "classes", 0);
	int max_boxes = option_find_int_quiet(options, "max", 1);
	char *pos_loss_type_s = option_find_str(options, "ploss", "l2");
	char *con_loss_type_s = option_find_str(options, "closs", "l2");
	char *strategy_type_s = option_find_str(options, "strategy", "yolo2");

	int total = option_find_int_quiet(options, "num", 1);
	int num = total;
	char *mask_lst = option_find_str(options, "mask", 0);
	int *mask = parse_yolo_mask(mask_lst, &num);

	layer l = make_focal_layer(params.batch, params.w, params.h, num, total, mask, classes, coords, max_boxes, pos_loss_type_s, con_loss_type_s, strategy_type_s);
	assert(l.outputs == params.inputs);

	l.focal_alpha = option_find_float_quiet(options, "alpha", 0.25);
	l.focal_gamma = option_find_float_quiet(options, "gamma", 2);

	l.jitter = option_find_float_quiet(options, "jitter", 0.2);
	l.rescore = option_find_int_quiet(options, "rescore", 0);

	l.size_low = option_find_float_quiet(options, "low", 0.0);
	l.size_high = option_find_float_quiet(options, "high", 10000000.0);
	l.class_low = option_find_int_quiet(options, "cls_low", 0);
	l.class_high = option_find_int_quiet(options, "cls_high", 98);

	l.ignore_thresh = option_find_float_quiet(options, "ignore_thresh", 0.5);
	l.pos_thresh = option_find_float_quiet(options, "pos_thresh", 0.5);
	l.neg_thresh = option_find_float_quiet(options, "neg_thresh", 0.5);
...
layer parse_mix(list *options, size_params params) {
	int coords = option_find_int_quiet(options, "coords", 4);
	int classes = option_find_int_quiet(options, "classes", 0);
	int num = option_find_int_quiet(options, "num", 1);
	int max_boxes = option_find_int_quiet(options, "max", 1);
	...
	layer l = make_mix_layer(params.batch, params.w, params.h, num, classes, coords, max_boxes, pos_loss_type_s, con_loss_type_s, strategy_type_s);
...
{
    net->batch = option_find_int_quiet(options, "batch",1);
    net->learning_rate = option_find_float_quiet(options, "learning_rate", .001);
    net->momentum = option_find_float_quiet(options, "momentum", .9);
    net->decay = option_find_float_quiet(options, "decay", .0001);
    int subdivs = option_find_int_quiet(options, "subdivisions",1);
	...
	// reused this parameter to choose the way to load data (originally used for multi-scales training)
    net->random = option_find_int_quiet(options, "random", 0);

    net->adam = option_find_int_quiet(options, "adam", 0);
    if(net->adam){
        net->B1 = option_find_float_quiet(options, "B1", .9);
        net->B2 = option_find_float_quiet(options, "B2", .999);
        net->eps = option_find_float_quiet(options, "eps", .0000001);
    }

    net->h = option_find_int_quiet(options, "height",360);
    net->w = option_find_int_quiet(options, "width",640);
    net->c = option_find_int_quiet(options, "channels",3);
    ...
    net->max_batches = option_find_int_quiet(options, "max_batches", 0);
}
...
network *parse_network_cfg(char *filename)
{
    ...
    while(n){
		...
		}else if (lt == FOCAL) {  // added
			l = parse_focal(options, params);
		}else if (lt == MIX) {  // added
			l = parse_mix(options, params);
		}else if (lt == YOLO) {
...
#endif
    fprintf(stderr, "Saving weights to %s...", filename);
