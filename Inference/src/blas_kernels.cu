...
*/
__global__ void upsample_kernel(size_t N, int batch, int forward, int c,  float ratio_w, float ratio_h, float *in, int in_w, int in_h, float *out, int out_w, int out_h) {
	size_t i = (blockIdx.x + blockIdx.y*gridDim.x)*blockDim.x + threadIdx.x;
	if (i >= N) return;
	int out_idx = i;
	int ow = i % out_w;
	...
	int in_idx = b*in_w*in_h*c + ic*in_w*in_h + ih*in_w + iw;

	if (forward) out[out_idx] += in[in_idx];
	else atomicAdd(in + in_idx, out[out_idx]);
}
extern "C" void upsample_gpu(int batch, int c, int forward, float *in, int in_w, int in_h, float *out, int out_w, int out_h) {
	size_t size = out_w*out_h*c*batch;
	const float ratio_w = (float)in_w / out_w;
	const float ratio_h = (float)in_h / out_h;
	upsample_kernel<<<cuda_gridsize(size), BLOCK>>>(size, batch, forward, c, ratio_w, ratio_h, in, in_w, in_h, out, out_w, out_h);
	check_error(cudaPeekAtLastError());
}
